...
An example of a supervised classification case.
...
def predict_example(model: LogisticRegression) -> None:
    ...
    new_case = new_case[FEATURE_COLS]
    predicted_prob: float = model.predict_proba(new_case)[0, 1]

    LOG.info(f"New case:\n{new_case}")
    LOG.info(f"Predicted probability of diabetes: {predicted_prob:.2%}")
...
def make_plots(df_clean: pd.DataFrame, model: LogisticRegression) -> None:
    ...
    LOG.info("Creating chart: BMI vs diabetes outcome (scatter plot)")
    ...
    scatter_plt.set_title("BMI vs Diabetes Outcome (CLOSE chart to continue)")
    scatter_plt.set_xlabel("Body Mass Index (BMI)")
    scatter_plt.set_ylabel("Diabetes Outcome (0 = No, 1 = Yes)")
